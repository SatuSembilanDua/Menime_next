...
const PageAnime = ({ slug }) => {
	...
	if (error) {
		const errorMsg =
			error.response.status == 404 ? "Data Not Found!" : "Something Went Wrong!"
		errorPage = <ErrorPage message={errorMsg} />
		// return
	}
	...
	return (
		<>
			...
			{errorPage ? (
				...
				>
					<ItemList episodes={episodes} isLoading={isLoading} slug={slug} />
				</div>
			)}
		</>
	)
